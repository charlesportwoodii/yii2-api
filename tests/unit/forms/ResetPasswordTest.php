...
class ResetPasswordTest extends \tests\codeception\Unit
{
    ...
    public function testInit()
    {
        $user = $this->register();
        $this->specify(
            'test init scenario', function () use ($user) {
    ...
    public function testReset()
    {
        $user = $this->register();
        $this->specify(
            'test reset scenario (with token)', function () use ($user) {
        ...
        $this->specify(
            'test reset scenario (with user)', function () use ($user) {
                $faker = Factory::create();
                $form = new ResetPassword(['scenario' => ResetPassword::SCENARIO_RESET]);
                $form->setUser($user);
                $token = Base32::encode(\random_bytes(64));
    ...
    public function testResetWithOTP()
    {
        $user = $this->register();
        $this->specify(
            'test that password cannot be reset if OTP is enabled', function () use ($user) {
        ...
        $this->specify(
            'tests password reset with valid OTP code', function () use ($user) {
                // Enable OTP on the account
                $this->getUser()->provisionOTP();
                $this->getUser()->enableOTP();
    ...
    public function testAuthenticatedResetScenario()
    {
        $user = $this->register();
        $this->specify(
            'test authenticated password reset', function () use ($user) {
                $faker = Factory::create();
        ...
        $user = $this->register();
        $this->specify(
            'test authenticated password reset with OTP', function () use ($user) {
                $faker = Factory::create();
                $form = new ResetPassword(['scenario' => ResetPassword::SCENARIO_RESET_AUTHENTICATED]);
