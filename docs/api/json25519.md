...
> Note that the sessions are encrypted with a public key provided by the client, and only provides forward secrecy per session (Token). Forward secrecy is not guaranteed for the duration of the session.
...
Encrypted sessions can be established by first sending a `GET` request to `/api/v1/server/otk`, which will return a one time keypair.

> For more information see [server/OneTimeKeys.md](server/OneTimeKeys.md).

This endpoint will provide you with a public key which you can use to sign your first request to `/api/v1/user/authenticate`. This flow looks as follows:
...
> If you want the response to be encrypted in reply, send `Accept: application/json+25519` as well. The response will be encrypted using a new session public key which will be provided in the response headers
...
In response, you will receive an enhanced token which provides support for encrypted sessions. A new `public` key, `signing` key, and `hash` are provided for all future requests moving forward. This information is valid for the duration of your Token.
...
```json
{
    "data": {
        ...
        "crypt": {
            "public": "YpFokbnNRjoZu8aVVZtU1vaqR9uPcc9Nu9HawsUWF28=",
            "signing": "ecYXfAwNVoS9ePn4xWhiJOdXQzr6LpJIeIn4AVju/Ug=",
            "signature": "MCuhvXhemaD1j4BbXhbIJS0Ch97n+rAQbYjIdvcHK/3KdJooUn9K0qiOdm+iIpr3tj6Gb5XhhdOwlDbPQ9MTB2KRaJG5zUY6GbvGlVWbVNb2qkfbj3HPTbvR2sLFFhdv",
